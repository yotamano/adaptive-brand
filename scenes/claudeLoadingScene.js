...
export async function setup(root) {
  // Ensure the root wrapper itself fills the viewport
  ...
  // Create an iframe that fills the scene wrapper and loads the demo
  const iframe = document.createElement('iframe');
  iframe.src = 'ben/claude-loading-demo.html';
  ...
  return {
    teardown() {
      ...
      keyForwarders.forEach(({ win, forward }) => {
        ...
        try {
          win.removeEventListener('keyup', forward);
        } catch (_) {}
      });
    }
  };
